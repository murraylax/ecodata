...
For World Bank data, you can download the same variable for multiple countries using the `get_ecodata_allcountries()` function.
...
The *ecodata* package includes even more functionality than what is described in this document. For routine tasks, this package allows you to accomplish a lot with *just three lines of code*. Here is an example illustrating credit card and mortgage interest rates in the United States.

```{r, warning = FALSE, message = FALSE}
# Line 1: Download data
intrates <- get_ecodata(c("https://fred.stlouisfed.org/series/TERMCBCCINTNS", 
                          "https://fred.stlouisfed.org/series/MORTGAGE30US"))
```
```{r, warning = FALSE, message = FALSE, fig.width=9}
# Line 2: Make a time series plot
ggplot_ecodata_ts(intrates, title = "Mortgage and Credit Card Interest Rates", plot.recessions = TRUE)
```
```{r, warning = FALSE, message = FALSE}
